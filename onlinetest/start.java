...
public class start {
    public DisplayImage() {
        JFrame frame = new JFrame("Zefinite Startup Employability Test");
        ...
        JButton blogin = new JButton("Proceed to the test");
        JLabel label = new JLabel();
        label.setIcon(new ImageIcon("C:\\onlinetest\\zefset.png"));// your image here
		label.setVerticalAlignment(JLabel.CENTER);
		label.setHorizontalAlignment(JLabel.CENTER);
		blogin.setSize(200,20);
...
for( i=0;i<2;i++){
if(puname.equalsIgnoreCase(name[i])==true){flagname=i;break;}
}
for( i=0;i<2;i++){
if(ppaswd.equalsIgnoreCase(roll[i])==true){flagroll=i;break;}
}
...
class Centerwindow extends JPanel{
	...
	public Centerwindow(){
		...
		if(TIMELIMIT >0)testresult = new JLabel("   You have only : "+TIMELIMIT+" minutes to complete.");   
...
/* mark_ques() method to highlight correct answers */
	...
	public void calculateResult(){
		...
		temp=(double)mark;
		if(temp/TOTAL*100 >=PASS) testresult.setText("  Well done , you passed");
		else testresult.setText("  Better luck next time " );
		student.setText(" Final score "+mark+" out of "+TOTAL+":  "+df.format(temp/TOTAL*100)+"%");
